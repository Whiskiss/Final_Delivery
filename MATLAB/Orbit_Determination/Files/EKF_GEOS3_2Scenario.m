...
for scn = 1:20 %21:size(scenario,1)
    clc

    clearvars -except scenario scn
     format long g
    %close all
    %%
    ...
    iStart = 1; %50
    iEnd = min(scenario{scn,1},size(data{1,4},1)); %iStart+50
    dataAll = {data{1,4}(iStart:iEnd,:)};
    timeIdx = scenario{scn,2}; %[1,3,5]; %[1,9,18];
    
% Noise added
if scenario{1,5}(1)
    for idx = 1:size(dataAll,2)
        dataAll{1,1}(:,7) = awgn(dataAll{1,1}(:,7),scenario{1,5}(2));
        dataAll{1,1}(:,8) = awgn(dataAll{1,1}(:,8),scenario{1,5}(2));
    end
end
...
%%
    ...
    for iData = 1:size(dataAll,2)
        ...
        P = zeros(6);
        for idx=1:3
            P(idx,idx)=scenario{scn,3}(1);
        end
        for idx=4:6
            P(idx,idx)=scenario{scn,3}(2);
        end
    ...
    %         Phi = Phi + W;
            ...
            Q = 0 * eye(size(Phi));
            val1 = scenario{scn,4}(1);
            val2 = scenario{scn,4}(2);
            if mod(idx,2) == scenario{scn,4}(3) %10
    ...
    save(sprintf('%i.mat',scn), 'data', 'dataAll');
