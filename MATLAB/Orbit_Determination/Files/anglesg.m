...
% geocentric inertial position ____________________________________________
...
while ((abs(rhoold2-rho2) > 1e-12) && (ll <= 0 ))
    ...
    if ( ll <= 8 )
        u = const.GM_Earth/magr2^3;
        rdot= dot(r2,v2)/magr2;
        udot= (-3*const.GM_Earth*rdot)/(magr2^4);
